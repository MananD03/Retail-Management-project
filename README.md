...
-- Both the ranking functions behave similarly for the total amount that the customer has paid so far but it behaves differently for the quantities that they have purchased so far. We can clearly see that for the quantities there are ties within customers and therefore, the dense_rank and rank assigns equal rank to them although dense rank sequentially assigns the rank to the customers while rank skips the rank/s every time there is a tie.
...
ORDER BY DATEDIFF(NOW(),Complaint_Date) DESC) AS _rank 
FROM Complaints 
...
-- 10) Divide the household customer into 3 segments: highPurchase, mediumPurchase and lowPurchase based on ranking of customers by their total purchase amount (first 25% in low, 25 to 75 medium and > 75% high) [Hint: Use Percent_Rank and ASC order first]. 
...
SELECT PurchasePower, COUNT(Customer_Id) AS CustomerCount
FROM (
    SELECT *,
    CASE 
    WHEN _rank < 0.25 THEN 'lowPurchase'
    ...
    END AS PurchasePower
    FROM (
        SELECT Customer_Id, Cust_Usage, 
        SUM(Amount_Paid) AS TotalPurchase,
        PERCENT_RANK() OVER(ORDER BY SUM(Amount_Paid)) AS _rank
        FROM ProductSalesFact
        WHERE Cust_Usage = 'Household'
        GROUP BY Customer_Id, Cust_Usage
    ) AS RankedHouseholdCustomers
) AS AggregatedData
GROUP BY PurchasePower;
...
SELECT PS.*, PD.Discount FROM ProductSalesFact AS PS 
JOIN ProductDim AS PD ON PS.Product_Id = PD.Product_Id 
ORDER BY CASE  WHEN Cust_Usage = "Household" 
THEN Amount_Paid ELSE Discount 
END;
